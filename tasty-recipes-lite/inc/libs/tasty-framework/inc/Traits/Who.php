 ...
 * Who trait.
 *
 * Checks if the current user matches the target specified for a given API item (an Inbox message or Sale).
 */
trait Who {
	...
	protected function matches_who( $who ) {
		...
		if ( self::matches_pro_tasty_product( $who ) ) {
			return true;
		}
	...
	/**
	 * See if the who targets an installed and licensed Pro Tasty product.
	 *
	 * @param array $who The target specified for a given API item (an Inbox message or Sale).
	 *
	 * @return bool True if the current user matches the target specified for a given API item (an Inbox message or Sale).
	 */
	private static function matches_pro_tasty_product( $who ) {
		$tasty_products = array(
			'recipes',
			'roundups',
			'links',
			'pins',
		...
		$matches = array_intersect( $tasty_products, $who );
		if ( ! $matches ) {
			return false;
		}

		foreach ( Factory::create_active_plugins() as $active_plugin ) {
			if ( self::plugin_meets_pro_requirements( $active_plugin ) ) {
				$slug = str_replace( 'tasty-', '', $active_plugin->get_menu_slug() );
	...
	/**
	 * See if the plugin meets the requirements for a Pro Tasty product.
	 *
	 * @param \Tasty\Framework\Abstracts\PluginInstaller $plugin The plugin to check.
	 *
	 * @return bool True if the plugin meets the requirements for a Pro Tasty product.
	 */
	private static function plugin_meets_pro_requirements( $plugin ) {
		return $plugin->has_framework() && ! $plugin->is_lite() && $plugin->is_licensed();
	}
	...
	private static function is_first_30() {
		$activation_timestamp = get_option( 'tasty_first_activation' );
